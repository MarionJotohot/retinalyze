...
async function updateDoctorData({ userId, profileFields, doctorFields }) {
  ...
  if (!docData)
    throw new Error(`No doctor record found for user_id: ${userId}`);
...
export function useEditDoctorQuery(authUserId) {
  ...
  return useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (profData && typeof setProfile === "function") {
        setProfile(profData);
      }

      // Patch react-query cache directly to avoid waiting for refetch
      if (profData) {
        queryClient.setQueryData(["profile", authUserId], profData);
      }
      if (docData) {
        queryClient.setQueryData(["doctor", authUserId], docData);
      }

      // Debug log while investigating
      console.log(
        "useEditDoctorQuery onSuccess - profData:",
        profData,
        "docData:",
        docData
      );
